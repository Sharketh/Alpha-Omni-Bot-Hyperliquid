import numpy as np
from scipy.signal import argrelextrema
from scipy.stats import linregress
from modules.config_loader import CONFIG

def get_slope(values):
    """
    Menghitung kemiringan (slope) dari sekumpulan nilai untuk menentukan tren.
    """
    try: 
        if len(values) < 2: return 0.0
        return linregress(np.arange(len(values)), values)[0]
    except: 
        return 0.0

def check_alignment(values):
    """
    Mengecek apakah titik-titik (puncak/lembah) sejajar dalam batas toleransi.
    Digunakan untuk konfirmasi Double Top/Bottom atau Rectangle.
    """
    if len(values) < 2: return False
    # Mengambil toleransi dari config, default 1.5%
    tol = CONFIG['patterns'].get('tolerance', 0.015)
    avg = np.mean(values)
    return all(abs(v - avg) / avg < tol for v in values)

def find_pattern(df):
    """
    Menganalisa dataframe untuk menemukan pattern chart berdasarkan 
    lokasi puncak (peaks) dan lembah (valleys).
    """
    if len(df) < 50: return None
    
    # Reset index agar perhitungan argrelextrema akurat secara posisi
    df_idx = df.reset_index(drop=True)
    n = 3 # Sensitivitas deteksi (jarak antar titik)
    
    # Deteksi Puncak dan Lembah Lokal
    df_idx['min_local'] = df_idx.iloc[argrelextrema(df_idx.low.values, np.less_equal, order=n)[0]]['low']
    df_idx['max_local'] = df_idx.iloc[argrelextrema(df_idx.high.values, np.greater_equal, order=n)[0]]['high']
    
    peaks = df_idx[df_idx['max_local'].notnull()]['max_local'].values
    valleys = df_idx[df_idx['min_local'].notnull()]['min_local'].values
    
    # Pastikan ada cukup data untuk membentuk pattern
    if len(peaks) < 3 or len(valleys) < 3: return None
    
    enabled = CONFIG['patterns']
    # Ambil kemiringan dari 4 titik terakhir untuk melihat struktur terbaru
    s_high = get_slope(peaks[-4:])
    s_low = get_slope(valleys[-4:])
    
    # --- LOGIKA DETEKSI PATTERN ---
    
    # 1. Ascending Triangle (Atas Flat, Bawah Naik)
    if enabled.get('ascending_triangle') and abs(s_high) < 0.0005 and s_low > 0.0002: 
        return 'ascending_triangle'
    
    # 2. Descending Triangle (Bawah Flat, Atas Turun)
    if enabled.get('descending_triangle') and abs(s_low) < 0.0005 and s_high < -0.0002: 
        return 'descending_triangle'
    
    # 3. Double Bottom (Dua Lembah Sejajar)
    if enabled.get('double_bottom') and check_alignment(valleys[-2:]): 
        return 'double_bottom'
    
    # 4. Double Top (Dua Puncak Sejajar)
    if enabled.get('double_top') and check_alignment(peaks[-2:]): 
        return 'double_top'
    
    # 5. Bull Flag (Channel Turun Tipis setelah kenaikan)
    if enabled.get('bull_flag') and -0.002 < s_high < -0.0002 and -0.002 < s_low < -0.0002: 
        return 'bull_flag'
    
    # 6. Bullish Rectangle (Konsolidasi Flat / Sideways)
    if enabled.get('bullish_rectangle') and abs(s_high) < 0.0005 and abs(s_low) < 0.0005: 
        return 'bullish_rectangle'
    
    return None
