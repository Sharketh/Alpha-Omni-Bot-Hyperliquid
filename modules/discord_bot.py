...
def generate_chart(df, symbol, pattern, timeframe):
    ...
    try:
        ...
        if 'MACD_h' in plot_df.columns:
            m_cols = ['#2ebd85' if v >= 0 else '#f6465d' for v in plot_df['MACD_h']]
            apds.append(mpf.make_addplot(plot_df['MACD_h'], type='bar', panel=1, color=m_cols, ylabel='MACD'))
...
def send_alert(data):
    ...
    try:
        ...
        # --- SCORES & ANALYSIS ---
        scores_txt = f"Tech: `{data['Tech_Score']}` | SMC: `{data['SMC_Score']}` | Quant: `{data['Quant_Score']}` | Deriv: `{data['Deriv_Score']}`"
        analysis_txt = (
        ...
        embed = {
            ...
            "fields": [
                ...
                {"name": "🏁 Targets", "value": f"TP1: `{format_price(data['TP1'])}` | TP2: `{format_price(data['TP2'])}` | TP3: `{format_price(data['TP3'])}`", "inline": False},
                ...
                {"name": "🧠 Context", "value": f"Bias: **{data.get('BTC_Bias', 'Neutral')}**", "inline": False}
            ],
        ...
        if r.status_code in [200, 201]:
            ...
            """, (symbol, data['Side'], data['Timeframe'], data['Pattern'], data['Entry'], data['SL'], data['TP1'], 
                  data['TP2'], data['TP3'], data.get('Reason',''), data['Tech_Score'], data['Quant_Score'], data['Deriv_Score'], 
                  data['SMC_Score'], data.get('Basis',0), data.get('BTC_Bias',''), data.get('Z_Score',0), data.get('Zeta_Score',0), 
                  data.get('OBI',0), data.get('Tech_Reasons',''), data.get('Quant_Reasons',''), data.get('Deriv_Reasons',''), 
                  smc_reasons_str, r.json().get('id'), r.json().get('channel_id')))
            conn.commit()
            release_conn(conn)
            return True
...
def update_status_dashboard():
    ...
    try:
        ...
        cur.execute("SELECT symbol, side, status, entry_hit_at, created_at FROM trades WHERE status NOT ILIKE '%Closed%' ORDER BY created_at DESC")
        trades = cur.fetchall()
        ...
        row = cur.fetchone()
        msg_id = row['value_text'] if row else None
